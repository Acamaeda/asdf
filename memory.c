...
// UArray stuff here for inlining
struct UArray_T {
...
inline void *fast_UArray_at(UArray_T uarray, int i) {
...
inline word valid_addr(word seg, word addr, Memory mem)
{
        ...
        Segment s = (Segment)fast_UArray_at(mem->segs, seg);
        if (addr >= s->size)
                return 0;
        return 1;
}
...
inline word new_seg(word size, Memory mem)
{
        ...
        s = (Segment) fast_UArray_at(mem->segs, id);
        s->size = size;
...
inline void copy_seg(word src, word dest, Memory mem)
{
        if (src >= mem->next_id)
                assert(0);
        Segment s1 = (Segment)fast_UArray_at(mem->segs, src);
        if (dest >= mem->next_id)
                assert(0);
        Segment s2 = (Segment)fast_UArray_at(mem->segs, dest);
...
inline void free_memory(Memory *memm)
{
        ...
        for (int i = 0; i < UArray_length(segs); i ++){
                if ((word)i < mem->next_id){
                        Segment s = (Segment)fast_UArray_at(mem->segs, i);
                        if (0 < s->size)
                                free(s->array);
                }
        }
